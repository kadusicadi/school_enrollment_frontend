        ...
        <div>
            <div className="flex flex-col">
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                                ...
                                <div className="flex items-center bg-gray-100 rounded-md px-3 py-2">
                                    ...
                                    {selectedOptions.length > 0 &&
                                        selectedOptions.map((option) => (
                                            <div
                                                key={option}
                                                className="bg-blue-500 text-white rounded-md mr-1 px-2 py-1"
                                            >
                                                {option}
                                            </div>
                                        ))}
                                </div>
                                <div className="absolute bg-white w-full border rounded-md mt-1">
                                    {courses.map((option) => (
                                        <div
                                            key={option._course_code}
                                            onClick={() => handleOptionClick(option._course_code)}
                                            className={`flex items-center justify-between px-3 py-2 cursor-pointer ${selectedOptions.includes(option._course_code)
                                                ? "bg-blue-500 text-white"
                                                : "hover:bg-gray-100"
                                                }`}
                                        >
                                            <div>{option.course_name}</div>
                                            {selectedOptions.includes(option._course_code) && (
                                                <div className="text-white bg-blue-500 rounded-full h-4 w-4 flex justify-center items-center">
                                                    ✓
                                                </div>
                                            )}
                                        </div>
                    ...
                    <input type="submit" value="Prijava" className="border rounded w-full py-2 px-3 mt-40 cursor-pointer" />
                </form>
            </div>
        </div>
    )
